...
class Auth
{
	...
	/**
	 * [__construct 构造方法]
	 * @DateTime 2020-02-18 09:13:53
	 */
	public function __construct()
	{
		if (Config::get('setting.auth_config')) {
			// 可设置配置项 auth_config, 此配置项为数组。
			$this->config = array_merge($this->config, Config::get('setting.auth_config'));
		}
	}
	/**
	 * [check 检查权限]
	 ...
	 * @param type int           如果type为1， condition字段就可以定义规则表达式
	 * @param mode string        执行check的模式，默认url模式即?号后面参数也校验
	...
	public function check($name, $uid, $type = 1, $mode = 'url', $relation = 'or')
	{
		...
		$list = []; //保存验证通过的规则名
		if ($mode == 'url') {
			// strtolower把所有字符转换为小写，serialize() 序列化函数，unserialize反序列化数据。
	...
	public function getAuthList($uid, $type)
	{
		...
		if ($this->config['auth_type'] == 2) { // 2为登录认证
			// 规则列表结果保存到session
