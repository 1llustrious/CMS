...
session_start();
define('PROJECT_PATH', str_replace('/public','',dirname(str_replace('\\', '/', __DIR__))));
if(stripos(dirname(str_replace('\\', '/', __DIR__)),'/public') !== false){
    $public = '/public';
}else{
    $public = '';
}
...
$config = array(
    ...
    'sqlDir'      => PROJECT_PATH . PUBLIC_DIR . '/install/database/',   //数据库所在目录
    'prefix'      => 'hh_',           //默认表前缀
    'sqlName'     => 'install',       //数据库文件名称
    'demoData'    => 'demo',          //演示数据文件名称
    'databaseUrl' => PROJECT_PATH . '/config/database.php', //database.php文件地址
    'account'     => 'admin',                     //默认账号
    'password'    => '123456',                    //默认密码
    'limit'       => '50',                        //安装时多少条数据一次翻页
    'h5ConfigUrl' => PROJECT_PATH . PUBLIC_DIR . '/install/wap/static/config.js',   //h5的config.js文件地址
);
// 数据库配置
$db_config = array(
    'DB_HOST'   => '127.0.0.1',
...
// 检测是否已安装
if (file_exists(PROJECT_PATH . PUBLIC_DIR . '/install.lock')) {
    $errorTitle = '系统已安装';
    $errorMsg   = '你已经安装过该系统，如需重新安装需要先删除 /install.lock';
...
if ($get == $config['endPage']) {
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        if ($_POST['type'] == '1') {
            ...
            $password = md6($_POST['admin_password']);
            ...
            $update_user_sql = "UPDATE ".$db['DB_PREFIX']." SET `name` = '{$account}',`password` = '{$password}' WHERE `name` = 'admin'";
            $link->query($update_user_sql);
            $return['data']['page']      = 1;
...
return [
    ...
    'connections'     => [
        'mysql' => [
            ...
            // 数据库表前缀
            'prefix'            => Env::get('database.prefix', '{$db['DB_PREFIX']}'),
...
h5;
    ...
    @touch(PROJECT_PATH . PUBLIC_DIR . '/install.lock');
}
/**
