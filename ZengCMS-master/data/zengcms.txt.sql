-- 数据库
CREATE database zengcms DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci;
use zengcms;

-- 1、配置类型表
DROP TABLE IF EXISTS `hh_config_type`;
CREATE TABLE `hh_config_type`(
`id` smallint(5) unsigned PRIMARY KEY AUTO_INCREMENT COMMENT '主键id',
`config_type_name` varchar(64) NOT NULL UNIQUE DEFAULT '' COMMENT '配置类型名称',
`description` varchar(255) NOT NULL DEFAULT '' COMMENT '描述',
`create_time` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '创建时间',
`update_time` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '最后一次修改时间',
`sort` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '排序',
`status` tinyint(1) unsigned NOT NULL DEFAULT '1' COMMENT '状态；1：显示；0：隐藏'
)ENGINE=MyISAM AUTO_INCREMENT=1 DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci COMMENT='配置类型表';

-- 2、配置表
DROP TABLE IF EXISTS `hh_config`;
CREATE TABLE `hh_config`(
`id` mediumint(8) unsigned PRIMARY KEY AUTO_INCREMENT COMMENT '配置id',
`ename` varchar(32) NOT NULL UNIQUE DEFAULT '' COMMENT  '配置英文名称',
`cname` varchar(32) NOT NULL UNIQUE DEFAULT '' COMMENT  '配置中文名称',
`form_type` varchar(16) NOT NULL DEFAULT 'input' COMMENT '表单类型；input：单行文本；radio：单选按钮；tags：标签；array：数组；markdown：编辑器；colorpicker：取色器；datetime：时间；selects：高级下拉多选；selecto：高级下拉单选；map：地图；region：地区；checkbox：复选框；select：下拉菜单；textarea：文本域；file：附件；editor：编辑器；picture：单图；piclist：多图；onefile：单文件；filelist：多文件；onevideo：单视频；videolist：多视频',
`config_type_id` smallint(5) unsigned NOT NULL COMMENT '配置类型id',
`values` text DEFAULT NULL COMMENT '可选值，以下两种都可以：
1、多个可选值用逗号隔开。注：必须加说明，例：值:说明,值:说明 
2、如果是枚举型，需要配置该项，一行一个选项，例: 
0:选项1 
2:选项2',
`value` text DEFAULT NULL COMMENT '默认值，多个用逗号隔开',
`remark` varchar(255) NOT NULL DEFAULT '' COMMENT '配置说明，如配置说明是个链接那么就在配置说明后面加@链接地址，链接地址必须带http://或https://',
`create_time` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '创建时间',
`update_time` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '最后一次修改时间',
`is_core_configuration` tinyint(1) unsigned NOT NULL DEFAULT '1' COMMENT '是否核心配置；1：是；0：否，是核心配置就不能删除',
`sort` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '排序',
`status` tinyint(1) unsigned NOT NULL DEFAULT '1' COMMENT '状态；1：显示；0：隐藏'
)ENGINE=MyISAM AUTO_INCREMENT=1 DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci COMMENT='配置表';

-- 3、管理员表
DROP TABLE IF EXISTS `hh_admin`;
CREATE TABLE `hh_admin`(
`id` int(10) unsigned PRIMARY KEY AUTO_INCREMENT COMMENT '管理员id',
`name` varchar(64) NOT NULL UNIQUE DEFAULT '' COMMENT '用户名',
`image` varchar(100) NOT NULL DEFAULT '' COMMENT '头像',
`photo_album` text DEFAULT NULL COMMENT '相册',
`file_one` varchar(100) NOT NULL DEFAULT '' COMMENT '单文件',
`file_list` text DEFAULT NULL COMMENT '多文件',
`video_one` varchar(100) NOT NULL DEFAULT '' COMMENT '单视频',
`video_list` text DEFAULT NULL COMMENT '多视频',
`password` char(32) NOT NULL DEFAULT '' COMMENT '密码',
`relname` varchar(32) NOT NULL DEFAULT '' COMMENT '真实姓名',
`sex` tinyint(1) unsigned NOT NULL DEFAULT '1' COMMENT '性别：1：男；2：女',
`phone` char(11) NOT NULL UNIQUE DEFAULT '' COMMENT '手机号码',
...
`sort` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '排序'
)ENGINE=MyISAM AUTO_INCREMENT=1 DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci COMMENT='管理员表';
...
`admin_ip` varchar(16) NOT NULL DEFAULT '' COMMENT '操作管理员IP',
...
)ENGINE=MyISAM AUTO_INCREMENT=1 DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci COMMENT='管理员操作日志主表';
...
`field_type` varchar(32) NOT NULL DEFAULT '' COMMENT '字段数据类型',
`field_value` text DEFAULT NULL COMMENT '原来字段值',
`current_field_value` text DEFAULT NULL COMMENT '当前字段值',
`comment` text DEFAULT NULL COMMENT '字段注释'
)ENGINE=MyISAM AUTO_INCREMENT=1 DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci COMMENT='管理员操作日志从表';

-- 6、管理员操作日志行为表
DROP TABLE IF EXISTS `hh_logs`;
CREATE TABLE `hh_logs` (
`id` int(10) unsigned NOT NULL PRIMARY KEY AUTO_INCREMENT COMMENT '主键id',
`url` varchar(255) NOT NULL DEFAULT '' comment "操作节点-模型和控制器和方法",
...
`operate_area` varchar(100) NOT NULL DEFAULT '' comment "操作地区"
)ENGINE=MyISAM AUTO_INCREMENT=1 DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci COMMENT='管理员行为日志表';
...
`title` char(64) NOT NULL DEFAULT '' COMMENT '规则中文名称',
`title_en` char(64) NOT NULL DEFAULT '' COMMENT '规则英文名称',
`name` varchar(255) NOT NULL DEFAULT '' COMMENT '规则唯一标识',
`type` tinyint(1) NOT NULL DEFAULT '1' COMMENT '如果type为1， condition字段就可以定义规则表达式',
`condition` char(255) NOT NULL DEFAULT '' COMMENT '规则附件条件,满足附加条件的规则,才认为是有效的规则',
`status` tinyint(1) NOT NULL DEFAULT '1' COMMENT '状态：为1正常，为0禁用',
`show` tinyint(1) unsigned NOT NULL DEFAULT '1' COMMENT '菜单是否显示；1：显示；0：不显示',
`icon` varchar(32) NOT NULL DEFAULT '' COMMENT '图标名称',
...
)ENGINE=MyISAM AUTO_INCREMENT=1 DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci COMMENT='规则(或菜单)表';

-- 8、用户组(或角色)表
DROP TABLE IF EXISTS `hh_auth_group`;
CREATE TABLE `hh_auth_group` (
`id` mediumint(8) unsigned NOT NULL PRIMARY KEY AUTO_INCREMENT COMMENT '主键id',
`title` char(100) NOT NULL DEFAULT '' COMMENT '用户组中文名称',
`remark` varchar(255) NOT NULL DEFAULT '' COMMENT '说明',
`status` tinyint(1) unsigned NOT NULL DEFAULT '1' COMMENT '状态：为1正常，为0禁用',
`sort` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '排序',
`rules` text DEFAULT NULL COMMENT '用户组拥有的规则id，多个规则","隔开，星号"*"代表拥有所有规则',
`create_time` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '创建时间',
`update_time` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '最后一次修改时间'
)ENGINE=MyISAM AUTO_INCREMENT=1 DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci COMMENT='用户组(或角色)表';
...
)ENGINE=MyISAM DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci COMMENT='用户组(或角色)明细表';
...
)ENGINE=MyISAM AUTO_INCREMENT=1 DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci COMMENT='系统节点表';
...
-- PRIMARY KEY (`id`),
KEY `index_auth_group` (`auth_id`) USING BTREE,
KEY `index_system_auth_node` (`node_id`) USING BTREE
)ENGINE=MyISAM AUTO_INCREMENT=1 DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci COMMENT='角色与节点关系表';
...
`title` varchar(100) UNIQUE NOT NULL DEFAULT '' COMMENT '模型(表单)名称',
`remark` varchar(255) NOT NULL DEFAULT '' COMMENT '简介',
`name` varchar(100) UNIQUE NOT NULL DEFAULT '' COMMENT '模型(表单)标识(即表名)只能使用英文且不能重复',
`fields` text DEFAULT NULL COMMENT '字段列表',
`extend` tinyint(1) unsigned NOT NULL DEFAULT '1' COMMENT '模型性质(目前仅支持1:文档模型,继承基础文档,0:独立模型)',
`list_template` varchar(100) NOT NULL DEFAULT '' COMMENT '列表页模板',
`index_template` varchar(100) NOT NULL DEFAULT '' COMMENT '频道封面页模板',
`article_template` varchar(100) NOT NULL DEFAULT '' COMMENT '文章内容页模板',
`article_rule` varchar(100) NOT NULL DEFAULT '' COMMENT '文章命名规则',
`list_rule` varchar(100) NOT NULL DEFAULT '' COMMENT '列表命名规则',
`create_time` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '创建时间',
`update_time` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '更新时间',
`status` tinyint(1) unsigned NOT NULL DEFAULT '1' COMMENT '状态(1:启用,0:禁用)',
`need_pk` tinyint(1) unsigned NOT NULL DEFAULT '1' COMMENT '新建表时增加的id字段是否是主键(1:是,0:否)',
`engine_type` varchar(25) NOT NULL DEFAULT 'MyISAM' COMMENT '数据库引擎(MyISAM;InnoDB;MEMORY;BLACKHOLE;MRG_MYISAM;ARCHIVE)',
`is_system` tinyint(1) unsigned NOT NULL DEFAULT '0' COMMENT '是否为系统模型,系统模型将不能删除(0:否,1:是)',
`sort` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '排序',
`setting` text DEFAULT NULL COMMENT '配置信息',
PRIMARY KEY (`id`)
)ENGINE=MyISAM AUTO_INCREMENT=1 DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci COMMENT='模型(表单)表';

-- 13、模型属性表(模型字段表)
DROP TABLE IF EXISTS `hh_attribute`;
CREATE TABLE `hh_attribute` (
`id` int(10) unsigned NOT NULL AUTO_INCREMENT COMMENT '主键自增ID,字段id',
`name` varchar(32) NOT NULL DEFAULT '' COMMENT '字段名称(即所属模型的表的字段名,英文字母)',
`title` varchar(100) NOT NULL DEFAULT '' COMMENT '字段注释(字段标题,用于表单显示)',
`field` varchar(100) NOT NULL DEFAULT '' COMMENT '字段定义',
`type` varchar(20) NOT NULL DEFAULT '' COMMENT '字段类型(用于表单中的展示方式)',
`value` varchar(255) NOT NULL DEFAULT '' COMMENT '默认值(字段的默认值)',
`remark` varchar(100) NOT NULL DEFAULT '' COMMENT '字段备注(用于表单中的提示)',
...
`extra` varchar(255) NOT NULL DEFAULT '' COMMENT '参数(布尔、枚举、多选字段类型的定义数据)
两种方式：
1、多个可选值用逗号隔开。注：必须加说明，例：值:说明,值:说明 
2、如果是枚举型，需要配置该项，一行一个选项，例: 
0:选项1 
...
`table_name` varchar(16) NOT NULL DEFAULT '' COMMENT '模型ID为0时，非模型表名',
...
`validate_rule` varchar(255) NOT NULL DEFAULT '' COMMENT '验证规则(根据验证方式定义相关验证规则)',
...
`auto_type` varchar(25) NOT NULL DEFAULT '' COMMENT '自动完成方式(function:函数,field:字段,string:字符串)',
`auto_rule` varchar(100) NOT NULL DEFAULT '' COMMENT '自动完成规则(根据完成方式订阅相关规则)',
`auto_time` tinyint(1) unsigned NOT NULL DEFAULT '0' COMMENT '自动完成时间(0:不完成,1:新增时,2:编辑时,3:始终)',
`sort` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '排序',
PRIMARY KEY (`id`),
KEY `model_id` (`model_id`)
)ENGINE=MyISAM AUTO_INCREMENT=1 DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci COMMENT='模型属性表(模型字段表)';
...
`typename` varchar(150) NOT NULL DEFAULT '' COMMENT '栏目名称',
`typename_en` varchar(150) NOT NULL DEFAULT '' COMMENT '英文栏目名称',
`name` varchar(255) NOT NULL DEFAULT '' COMMENT '栏目标识(英文字母)',
...
`typedir` varchar(150) NOT NULL DEFAULT '' COMMENT '文件保存目录(栏目目录)',
`list_template` varchar(100) NOT NULL DEFAULT '' COMMENT '列表页模板',
`index_template` varchar(100) NOT NULL DEFAULT '' COMMENT '频道封面页模板',
`article_template` varchar(100) NOT NULL DEFAULT '' COMMENT '文章内容页模板',
`article_rule` varchar(100) NOT NULL DEFAULT '' COMMENT '文章命名规则',
...
`seotitle_en` varchar(255) NOT NULL DEFAULT '' COMMENT '英文SEO标题',
`keywords` varchar(255) NOT NULL DEFAULT '' COMMENT '栏目关键词',
`keywords_en` varchar(255) NOT NULL DEFAULT '' COMMENT '英文栏目关键词',
`description` varchar(255) NOT NULL DEFAULT '' COMMENT '栏目描述',
`description_en` varchar(255) NOT NULL DEFAULT '' COMMENT '英文栏目描述',
`content` text DEFAULT NULL COMMENT '栏目内容',
`content_en` text DEFAULT NULL COMMENT '英文栏目内容',
...
`create_time` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '创建时间',
`update_time` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '更新时间',
`thumb` varchar(100) NOT NULL DEFAULT '' COMMENT '栏目图片',
`isfirst` tinyint(1) unsigned NOT NULL DEFAULT '0' COMMENT '是否默认显示第一个子栏目内容(1:是,0:否)',
`sort` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '排序',
PRIMARY KEY (`id`)
)ENGINE=MyISAM AUTO_INCREMENT=1 DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci COMMENT='文章类型表(栏目表)';
...
)ENGINE=MyISAM AUTO_INCREMENT=1 DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci COMMENT='栏目权限表';
...
`flags` char(16) NOT NULL DEFAULT '' COMMENT '文档自定义属性(h:头条,c:推荐,f:幻灯,a:特荐,s:滚动,b:加粗,p:图片,j:跳转)',
`tags` varchar(255) NOT NULL DEFAULT '' COMMENT 'TAG标签(","号分开，单个标签小于12字节)',
`thumb` varchar(100) NOT NULL DEFAULT '' COMMENT '图标',
...
`status` char(10) NOT NULL DEFAULT '1' COMMENT '数据状态(-1:删除,0:隐藏-禁用,1:显示-正常,2:待审核,3:草稿)',
`sort` int(10) NOT NULL DEFAULT '0' COMMENT '排序',
`create_time` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '创建时间',
`update_time` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '更新时间',
PRIMARY KEY (`id`)
)ENGINE=MyISAM AUTO_INCREMENT=1 DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci ROW_FORMAT=DYNAMIC COMMENT '基础文档表(模型)';
...
`title` varchar(255) NOT NULL DEFAULT '' COMMENT '组类别名',
`issystem` tinyint(1) unsigned NOT NULL DEFAULT '0' COMMENT '是否系统内置',#这里好像多余的
`tid` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '字段id',
`fields` varchar(255) NOT NULL DEFAULT '' COMMENT '字段名称',
`sort` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '排序',
PRIMARY KEY (`id`)
)ENGINE=MyISAM AUTO_INCREMENT=1 DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci ROW_FORMAT=DYNAMIC COMMENT '联动类别表';

-- 17、tag标签表
DROP TABLE IF EXISTS `hh_tag`;
CREATE TABLE `hh_tag`(
`id` int(10) unsigned PRIMARY KEY AUTO_INCREMENT COMMENT '主键id',
`name` varchar(32) UNIQUE NOT NULL DEFAULT '' COMMENT '标签名称',
`color` varchar(64) NOT NULL DEFAULT '' COMMENT '标签颜色',
`seotitle` varchar(255) NOT NULL DEFAULT '' COMMENT 'seo标题',
`keywords` varchar(255) NOT NULL DEFAULT '' COMMENT '关键字',
`description` varchar(255) NOT NULL DEFAULT '' COMMENT '描述',
`view` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '浏览量',
`target` varchar(10) NOT NULL DEFAULT '_self' COMMENT 'HTML<a>标签的target属性(_blank:在新窗口中打开被链接文档,_self:默认。在相同的框架中打开被链接文档,_parent:在父框架集中打开被链接文档,_top:在整个窗口中打开被链接文档,framename:在指定的框架中打开被链接文档)',
`rel` varchar(10) NOT NULL DEFAULT '' COMMENT 'HTML<a>标签的rel属性，nofollow：Google使用"nofollow"，用于指定 Google 搜索引擎不要跟踪链接。external：此属性的意思是告诉搜索引擎，这个链接不是本站链接。noopener noreferrer：在新打开的页面（baidu）中可以通过window.opener获取到源页面的部分控制权，即使新打开的页面是跨域的也照样可以',
`status` tinyint(1) unsigned NOT NULL DEFAULT '1' COMMENT '状态(1:显示,0:隐藏)',
`sort` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '排序',
`create_time` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '创建时间',
`update_time` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '修改时间'
)ENGINE=MyISAM AUTO_INCREMENT=1 DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci COMMENT='tag标签表';
...
)ENGINE=MyISAM DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci COMMENT='tagmap表';
...
`file_name` varchar(128) UNIQUE NOT NULL DEFAULT '' COMMENT '文件名称',
`file_type` char(6) NOT NULL DEFAULT 'image' COMMENT 'image：图片，file：文件，video：视频',
`sort` int(10) unsigned NOT NULL DEFAULT '100' COMMENT '排序',
`create_time` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '创建时间',
`update_time` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '修改时间',
PRIMARY KEY (`id`)
)ENGINE=MyISAM AUTO_INCREMENT=1 DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci COMMENT='附件表';
...
`name` varchar(32) UNIQUE NOT NULL DEFAULT '' COMMENT '分组名称',
`is_default` tinyint(1) unsigned NOT NULL DEFAULT '0' COMMENT '是否默认(0否,1是)',
`sort` int(10) unsigned NOT NULL DEFAULT '100' COMMENT '排序',
`create_time` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '创建时间',
`update_time` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '修改时间',
PRIMARY KEY (`id`)
)ENGINE=MyISAM AUTO_INCREMENT=1 DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci COMMENT='附件分组表';
...
`name` varchar(64) NOT NULL DEFAULT '' COMMENT '插件名或标识',
`images` varchar(255) NOT NULL DEFAULT '' COMMENT '封面',
`group` varchar(32) NOT NULL DEFAULT '' COMMENT '组别',
`description` varchar(255) NOT NULL DEFAULT '' COMMENT '插件描述',
`author` varchar(32) NOT NULL DEFAULT '' COMMENT '作者',
`version` varchar(32) NOT NULL DEFAULT '' COMMENT '版本号',
`require` varchar(64) NOT NULL DEFAULT '' COMMENT '需求版本',
`website` varchar(255) NOT NULL DEFAULT '' COMMENT '演示地址',
...
KEY `name` (`name`),
KEY `status` (`status`)
)ENGINE=MyISAM AUTO_INCREMENT=1 DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci COMMENT='公用插件表';
...
PRIMARY KEY (`id`),
UNIQUE KEY `name` (`name`)
)ENGINE=MyISAM AUTO_INCREMENT=1 DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci COMMENT='插件钩子';

-- 23、碎片位表
DROP TABLE IF EXISTS `hh_debris_pos`;
CREATE TABLE `hh_debris_pos` (
`id` mediumint(8) unsigned NOT NULL PRIMARY KEY AUTO_INCREMENT COMMENT '主键id',
`name` varchar(64) UNIQUE NOT NULL DEFAULT '' COMMENT '碎片位名称',
`description` varchar(255) NOT NULL DEFAULT '' COMMENT '碎片位描述',
`sort` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '排序'
)ENGINE=MyISAM AUTO_INCREMENT=1 DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci COMMENT='碎片位表';
...
`description` varchar(255) NOT NULL DEFAULT '' COMMENT '描述',
`content` text DEFAULT NULL COMMENT '碎片内容',
`status` tinyint(1) unsigned NOT NULL DEFAULT '1' COMMENT '状态(1:显示,0:隐藏)',
`sort` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '排序',
`create_time` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '创建时间',
`update_time` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '修改时间',
PRIMARY KEY (`id`)
)ENGINE=MyISAM DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci COMMENT='碎片表';
...
`sort` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '排序',
PRIMARY KEY (`id`)
)ENGINE=MyISAM AUTO_INCREMENT=1 DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci ROW_FORMAT=DYNAMIC COMMENT='广告位表';
...
`url` varchar(255) NOT NULL DEFAULT '' COMMENT '广告链接地址',
`thumb` varchar(100) NOT NULL DEFAULT '' COMMENT '缩略图',
`description` varchar(255) NOT NULL DEFAULT '' COMMENT '广告描述',
`content` text DEFAULT NULL COMMENT '广告内容',
`timeset` tinyint(1) unsigned DEFAULT '0' COMMENT '0永不过期，1限定时间内',
...
`update_time` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '修改时间',
PRIMARY KEY (`id`)
)ENGINE=MyISAM AUTO_INCREMENT=1 DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci ROW_FORMAT=DYNAMIC COMMENT='广告表';

-- 27、友情链接类型表
DROP TABLE IF EXISTS `hh_link_type`;
CREATE TABLE `hh_link_type` (
`id` smallint(5) unsigned NOT NULL PRIMARY KEY AUTO_INCREMENT COMMENT '自增ID',
`name` varchar(64) UNIQUE NOT NULL DEFAULT '' COMMENT '类型名称',
`description` varchar(255) NOT NULL DEFAULT '' COMMENT '描述',
`sort` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '排序'
)ENGINE=MyISAM AUTO_INCREMENT=1 DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci COMMENT='友情链接类型表';
...
`name` varchar(60) NOT NULL DEFAULT '' COMMENT '链接名称',
`thumb` varchar(100) NOT NULL DEFAULT '' COMMENT '缩略图',
...
`sort` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '排序'
)ENGINE=MyISAM AUTO_INCREMENT=1 DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci COMMENT='友情链接表';

-- 国/省/市/区、县表
DROP TABLE IF EXISTS `hh_region`;
CREATE TABLE `hh_region` (
`id` int(7) unsigned NOT NULL UNIQUE COMMENT 'ID(编码)',
`name` varchar(40) NOT NULL DEFAULT '' COMMENT '省市区名称',
`pid` int(7) unsigned NOT NULL DEFAULT '0' COMMENT '上级ID(编码)',
`shortname` varchar(40) NOT NULL DEFAULT '' COMMENT '简称',
`leveltype` tinyint(1) unsigned NOT NULL DEFAULT '3' COMMENT '级别(0:中国,1:省份,2:市,3:区、县)',
`citycode` varchar(7) NOT NULL DEFAULT '' COMMENT '城市代码',
`zipcode` varchar(7) NOT NULL DEFAULT '' COMMENT '邮编',
`lng` varchar(20) NOT NULL DEFAULT '' COMMENT '经度',
`lat` varchar(20) NOT NULL DEFAULT '' COMMENT '纬度',
`pinyin` varchar(40) NOT NULL DEFAULT '' COMMENT '拼音',
`status` enum('0','1') DEFAULT '1' COMMENT '状态',
`sort` int(10) unsigned NOT NULL DEFAULT '0' COMMENT '排序'
)ENGINE=MyISAM DEFAULT CHARSET=utf8 COLLATE=utf8_general_ci COMMENT='国/省/市/区、县表';
-- 地区表数据
