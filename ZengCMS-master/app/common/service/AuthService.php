...
class AuthService
{
    ...
    protected $config = [
        ...
        'system_admin'     => 'admin',            // 用户表
        // 'system_auth'      => 'system_auth',   // 权限表
        'system_auth'      => 'auth_group',       // 权限表
    ...
    public function checkNode($node = null)
    {
        ...
        // 判断是否加入节点控制，优先获取缓存信息
        $nodeInfo = Db::name($this->config['system_node'])->where(['node' => $node])->find();
        if (empty($nodeInfo)) {
            return false;
        }
        ...
        // 用户验证，优先获取缓存信息
        $adminInfo = Db::name($this->config['system_admin'])->where('id', $this->adminId)->find();
        $adminInfo['auth_ids'] = Db::name('auth_group_access')->where('uid',$this->adminId)->column('group_id auth_ids');
